...
const blog3 = {
  ...
  description:
    "There is no shortcut to success, but if you are willing to learn and put in some effort, you will surely get all the accomplishment you deserve. Android development is easy, but becoming a fortune Android developer and exiting out from the rest is not. You are required to put a lot of hard work, affection, adherence, and determination to become extreme at this job. To help you to become a better Android developer, I integrated some extremely useful pro tips from my experience. SO to all those who have decided to jump into Android development or have been working on it for a while, these tips will be suitable for you. Also, know the important tips to know before developing a mobile app.",
  category: "App Development",
  image: "https://karmill.s3.us-east-1.amazonaws.com/profile_pictures/t1.jpg",
  readTime: "5 Minutes",
  quote:
    "Looking for a company to develop Android Mobile Apps! Hire Quantum It Innovation a prominent Android app development company.",
...
const blog5 = {
  ...
  readTime: "5 Minutes",
  quote:
    "Use relevant and popular keywords in your app’s title and description to improve search visibility.",
  image2:
    "https://karmill.s3.us-east-1.amazonaws.com/profile_pictures/dm2.jpeg",
};
...
exports.CreateBlog = async (req, res, next) => {
  ...
  if (blogImage.size > maxSize || blogImage2.size > maxSize) {
    return res.status(400).json({
      success: false,
      message: "Each image size should not exceed 1MB.",
    });
  }
  ...
  try {
    ...
    return next(
      new ErrorHandler(`Error while saving the blog: ${e.message}`, 500)
    );
  }
};

exports.GetBlog = async (req, res, next) => {
  const { id, resultPerPage, currentPage, custom_url } = req.query;
  let limit = 10;
  let offset = 0;
  ...
  try {
    ...
    if (id) {
      ...
      blogs = await BlogModel.findOne({
        where: where(fn("TRIM", col("custom_url")), url),
      });
    } else {
    ...
    return next(
      new ErrorHandler(`Error while fetching blogs: ${e.message}`, 500)
    );
  }
};
...
exports.DeleteBlog = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new ErrorHandler(`Error while deleting the blog: ${e.message}`, 500)
    );
  }
};
